...
const routes = (app) => {
        ...
        db.query('SELECT * FROM users where `email` = ?',[email], (err, rows, fields) => {
          ...
          if (!rows.length) {
            db.query('INSERT INTO users SET ?', {email, name, votes_count: 0, fb_id}, (err, result) => {
              if (err) throw err
              console.log("user inserted", result.id);
              return res.json({
                id: result.insertId,
                fb_id: fb_id,
                name: name,
            ...
            console.log("user exists", rows[0].id);
            return res.json({
              id: rows[0].id,
              fb_id: rows[0].fb_id,
              name: rows[0].name,
  ...
  api.post('/new_vote', (req,res) => {
    ...
    const friends_ids = req.body.friends.split(',');
    ...
    db.query('INSERT INTO votes SET ?', {
      ...
      for(let friend in friends_ids) {
        db.query('INSERT INTO notifications SET ?', {
          ...
          if (!err) console.log("notifications inserted", result.insertId);
